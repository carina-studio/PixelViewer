...
- `ObservableProperty<T>` fields are named `XxxProp` (e.g. `ColorSpaceProp`), **never** `XxxProperty`, regardless of visibility. Reason: Avalonia 12 compiled bindings resolve any **public** static field named `<Member>Property` on the bound type as an `AvaloniaProperty` and fail compilation when it is not one (`AVLN2000: … doesn't inherit from AvaloniaProperty<T>`). A non-public field of that name compiles, so the rule is stricter than the compiler needs; the `Prop` suffix is applied to all visibilities for consistency. `AvaloniaProperty` fields on controls keep the standard `XxxProperty` suffix — that convention is required by Avalonia and does not conflict. `Session` and `Workspace` were renamed with the Avalonia 12 upgrade, so no `XxxProperty`-suffixed `ObservableProperty` field is left.
...
### XAML / Avalonia
- **Compiled vs. reflection bindings.** This repo is on Avalonia 12 (`AvaloniaVersion` in `Directory.Build.props`), where compiled bindings are the default and a `{Binding}` with no `x:DataType` in scope is a compile error (`AVLN2100`). Prefer compiled bindings as much as possible. `x:CompileBindings="True"` becomes redundant (compiled is the default) and is omitted. `x:CompileBindings="False"` is **not allowed** — it switches compiled bindings off for an entire scope, taking every binding in that scope down with the one that needed it; when a reflection binding genuinely is needed, opt that **single** binding out with the dedicated `{ReflectionBinding …}` markup extension instead. Carina Studio's AppSuiteBase framework states the same rule — see the `AGENTS.md` of the [AppSuiteBase repository](https://github.com/carina-studio/AppSuiteBase). To reach a member the starting data type does not carry — the outer `DataContext` behind a `#name` or `$parent[T]`, or a `Source` whose static type is a base class — cast the root of the path: `{Binding #root.((appViewModels:Session)DataContext).IsHistogramMeanMarkerVisible}`, `{Binding Source={x:Static app:App.Current}, Path=(as:AppSuiteApplication).IsDebugMode}`. `x:DataType` on the binding itself is ignored when the binding has a `Source`.
- **A member bound from XAML must never be `private`.** A compiled binding to a private CLR property whose `AvaloniaProperty` field is also private **compiles, then silently binds nothing** — the target keeps its default value, with no error at build time and no message at run time. Avalonia 11's reflection binding resolved such a member through the registered-property-by-name lookup, which ignores accessibility, so this is a silent regression rather than a build break: the Avalonia 12 upgrade turned the rendered image invisible and emptied the histogram view. Making either the CLR property or the `…Property` field non-private is enough (`internal` works, since the XAML lives in the same assembly), but declare the **CLR property `public`** with the usual `///` doc, so that what the XAML consumes reads as the control's API. This governs bindings only, a method bound as a `Command` included; an **event handler wired by attribute** (`Click="OnFooClick"`, `PointerPressed="OnBarPressed"`) stays `private` as usual, because the compiler emits that wiring inside the class itself.
