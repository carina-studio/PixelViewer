...
static class FileFormatParsers
{
    ...
    public static async Task<Profiles.ImageRenderingProfile?> ParseImageRenderingProfileAsync(IImageDataSource source, CancellationToken cancellationToken)
    {
            ...
            && FileFormats.TryGetFormatsByFileName(fileSource.FileName, out var fileFormats))
        {
            ...
            {
                if (parsers.TryGetValue(fileFormat, out var parser))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        logger?.LogWarning(ex, "Unable to parse '{fileName}' as {fileFormat} file", fileSource.FileName, parser.FileFormat);
                    }
                }
            }
            if (remainingParsers.IsEmpty())
            {
                logger?.LogTrace("Unable to identify file format of '{fileName}'", fileSource.FileName);
                return null;
            }
        }
        ...
        try
        {
            foreach (var parser in parsers.Values)
            {
                ...
                {
                    await Task.Run(() => stream.Position = 0, CancellationToken.None);
                }
            }
        }
        finally
